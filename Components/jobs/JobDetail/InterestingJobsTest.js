      ...
      <ul role="list" className="divide-y divide-gray-200">
        {props.positions &&
          props.positions?.slice(0, 4).map((position) => (
            <li key={position.id}>
              <a
                href={`/new-jobs-test/${useRemoveSpaceNew(
                  position.positionTitle
                )}/${useRemoveSpaceNew(
                  position.CompanyName
                )}/${useRemoveSpaceNew(position.locationName)}/${
                  position.JobId
                }`}
              ...
              >
                <div className="px-6 py-4">
                  <div className="flex items-center justify-between">
                    <p className="truncate text-sm font-medium text-indigo-600">{`${position.positionTitle}-${position.Category}`}</p>
