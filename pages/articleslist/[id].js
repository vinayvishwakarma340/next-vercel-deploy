import Head from "next/head";
import { useRouter } from "next/router";
import { useEffect } from "react";
import { useState } from "react";
import BreadCrumbs from "../../Components/BreadCrumbs";
import ArticleCard from "../../Components/CardUI/ArticleCard";
import Footer from "../../Components/Footer";
import GoogleAd_300x250 from "../../Components/GoogleAds/GoogleAd_300x250";
import GoogleAd_728x90 from "../../Components/GoogleAds/GoogleAd_728x90";
import MainHeader from "../../Components/MainHeader";
import NewPagination from "../../Components/NewPagination";
import FreelancerWidget from "../../Components/Widget/FreelancerWidget";
import SecondryWidget from "../../Components/Widget/SecondryWidget";
import { SearchArticlesResponse } from "../api/articleApi";
import { ThreeDots } from "react-loader-spinner";

import * as gtag from "../../lib/gtag";


const FeaturedArticles = (props) => {

  const [article, setArticle] = useState([]);
  const [page, setPage] = useState(parseInt(props.props.page));
  const [btnLoading, setBtnLoading] = useState(false);
  const [maxPage, setMaxPage] = useState(1);
  const [category, setCategory] = useState("");
  const [count, setCount] = useState(props.props.articledata.count);
  const router = useRouter();
  const id = router.query.id;
  const search = router.query.search;

  const [searchArticle, setSearchArticle] = useState(router.query.search || "");
  const freelancerWidgetData = {
    heading: "Work Remotely",
    description: " Get a network of certified and experienced freelancers",
    buttonText: "Hire now ",
  };

  useEffect(() => {
    setArticle(props.props.articledata);
    setPage(parseInt(props.props.page));
  }, [props.props.page, search]);

  useEffect(() => {
    setMaxPage(Math.ceil(count / 24));
  }, [count]);
  const ArticleEvent = () => {
    {
      article?.data?.map((item) => {
        gtag.event({
          action: item.title,
          category: "Article Impressions",
          label: item.ArticleId,
        });
      });
    }
  };
  useEffect(() => {
    ArticleEvent();
  }, [article?.data]);

  const pageChange = (val) => {
    let goodUrl = router.asPath.split("?");
    if (category) {
      router.push(`/${goodUrl[0]}?page=${val}&category=${category}`);
    } else {
      router.push(`/${goodUrl[0]}?page=${val}`);
    }
  };


  const headingHandler = (pageName) => {
    const parts = pageName.split("-");
    const [part1, part2, part3] = parts;

    const capitalize = (str) => str.charAt(0).toUpperCase() + str.slice(1).toLowerCase();

    if (part3 === "hr" || part3 === "her") {
      return (
        capitalize(part1) + " " +
        capitalize(part2) + " " +
        part3.toUpperCase()
      );
    }

    return (
      `${capitalize(part1)} ${capitalize(part2)}`
    );
  };


  const searchArticlApi = (e) => {
    e.preventDefault();
    setBtnLoading(true);
    var myHeaders = new Headers();
    myHeaders.append(
      "Authorization",
      process.env.API_TOKEN_AUTH_SERVER
    );
    myHeaders.append("Content-Type", "application/json");

    var raw = JSON.stringify({
      SearchCategory: "Article",
      SearchTerm: searchArticle,
    });

    var requestOptions = {
      method: "POST",
      headers: myHeaders,
      body: raw,
      redirect: "follow",
    };

    fetch(
      `${process.env.Live_API_URL}/v1/api/apiTimes/SearchByCategory`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        if (result.status_code === 200) {
          setArticle(result);
          setCount(result.datacount || 0);
          // setData(result.data);
          // setLoading(false);
        }
      })
      .catch((error) => console.log("error", error))
      .finally(() => setBtnLoading(false));
  };

  const MetaTitle = {
    "recommended-read": {
      H1: "Recommended Read",
      Title: "Latest articles for Recommended Read - timesascent.com",
      Meta: "Read articles on Jobs, Corporate Life, Talents & Careers & Global Research and more."
    }, "featured-articles": {
      H1: "Featured Articles",
      Title: "People’s perspective on Trending topics and more - timesascent.com",
      Meta: "At timesascent.com, you may read renowned people's perspectives on Career, Workplace, Trending Topics, and more."
    }, "career-development": {
      H1: "Career Development",
      Title: "Latest insights on career development – timesascent.com",
      Meta: "Get latest insights on career growth, development at timesascent.com"
    }, "voice-of-hr": {
      H1: "Voice of HR",
      Title: "HR Advice from Industry Experts",
      Meta: "Industry Experts share their thoughts on latest trends & topics."
    }, "trending-articles": {
      H1: "Trending Articles",
      Title: "Discover articles about skill development and upskilling.",
      Meta: "Explore articles related to upskilling and skill development – timesascent.com"
    }, "recommended-hr-read": {
      H1: "Recommended HR Read",
      Title: "Viewpoint of HR Professionals on latest topics.",
      Meta: "Read the perspectives of Top-Level HR Professionals – timesascent.com"
    }, "featured-hr-content": {
      H1: "Featured HR Content",
      Title: "Human Resources Outlook from Top HR Leaders.",
      Meta: "Learn more about Human Resource from the Top HR Leaders – timesascent.com"
    }, "articles-for-her": {
      H1: "Trending Articles for Women",
      Title: "Keep up with the most popular articles for women.",
      Meta: "Stay upto date with the most trending articles for women – timesascent.com"
    }, "inspirational-stories": {
      H1: "Inspirational Stories ",
      Title: "Life inspiring stories of Women Leaders",
      Meta: "Read the life-changing stories of women leaders."
    }, "gmi-articles": {
      H1: "Great Manager Institute",
      Title: "Top industry leaders' perspectives on current issues",
      Meta: "Viewpoint on current topics from Top Leaders of the industry."
    }
  }

  const pages = [
    {
      name: "Article List",
      current: false,
    },
    {
      name: MetaTitle[router.query.id]?.H1 || "",
      current: true,
    },
  ];
  return (
    <div>
      <Head>
        <meta charSet="utf-8" />
        <title>{MetaTitle[router.query.id]?.Title || "Latest articles for Recommended Read - timesascent.com"} </title>
        <meta
          name="description"
          content={MetaTitle[router.query.id]?.Meta || "Read articles on Jobs, Corporate Life, Talents & Careers & Global Research and more."}
        />

        <link
          rel="canonical"
          href={`https://timesascent.com/articleslist/${id}`}
        />
        <meta name="viewport" content="initial-scale=1.0, width=device-width" />

        <link
          rel="icon"
          type="image/png"
          sizes="32x32"
          href="https://timesascent.com/Times_Ascent_Icon.png"
        />
        <link
          rel="icon"
          type="image/png"
          sizes="16x16"
          href="https://timesascent.com/Times_Ascent_Icon.png"
        />
        <meta
          property="og:title"
          content={`Latest articles for Recommended Read - timesascent.com`}
        />
        <meta property="og:type" content="website" />
        <meta
          property="og:description"
          content={` Recommended Read epaper articles from Times of india newspaper - timesascent.com `}
        />
        <meta
          property="og:url"
          content={`https://timesascent.com/articleslist/${id}`}
        />
        <meta
          property="og:image"
          content="https://timesascent.com/newimages/main/times_ascent_logo.svg"
        />
        <meta
          property="twitter:image"
          content="https://timesascent.com/newimages/main/times_ascent_logo.svg"
        />
        <meta
          property="twitter:title"
          content={`Latest articles for Recommended Read - timesascent.com`}
        />
        <meta
          property="twitter:description"
          content={` Recommended Read epaper articles from Times of india newspaper - timesascent.com `}
        />
        <meta property="twitter:card" content="summary" />
        <meta
          property="twitter:site"
          content="https://twitter.com/@timesascent"
        />
        <meta name="mobile-web-app-capable" content="yes" />
        <meta property="og:image:width" content="200" />
        <meta property="og:image:height" content="200" />
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              "@context": "https://schema.org",
              "@type": "BreadcrumbList",
              itemListElement: [
                {
                  "@type": "ListItem",
                  position: 1,
                  name: "Home",
                  item: "https://timesascent.com/",
                },

                {
                  "@type": "ListItem",
                  position: 2,
                  name: MetaTitle[router.query.id]?.H1,
                  item: `https://timesascent.com${router.asPath}`,

                },
              ],
            }),
          }}
        />
      </Head>

      <MainHeader />
      <div className="  py-4 md:py-6 lg:py-8 container">
        <BreadCrumbs data={pages} />
        <div className="md:flex my-4 justify-between  items-center">
          <h1 className="text-xl   py-4  capitalize font-bold text-gray-900 ">
            {searchArticle ? "Search Article" : MetaTitle[router.query.id]?.H1 || ""}
          </h1>
          <form className="my-2 md:flex " onSubmit={searchArticlApi}>
            <div className="mb-1 md:mb-0 md:w-72">
              <input
                id="search"
                name="search"
                type="search"
                placeholder="Search "
                onChange={(e) => setSearchArticle(e.target.value)}
                value={searchArticle}
                className="block w-full appearance-none rounded-md border border-gray-300 px-3 py-2 placeholder-gray-400 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 "
              />
            </div>

            <button
              type="submit"
              className="md:ml-1 flex justify-center items-center rounded-md w-full md:w-24   border border-transparent bg-timesPurple py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-timesPurple focus:outline-none focus:ring-2 focus:ring-timesPurple focus:ring-offset-2"
            >
              {btnLoading ? (
                <ThreeDots
                  height="20"
                  width="30"
                  radius="9"
                  color="white"
                  ariaLabel="three-dots-loading"
                  wrapperStyle={{}}
                  wrapperClassName=""
                  visible={true}
                />
              ) : (
                "Search"
              )}
            </button>
          </form>
        </div>

        <div className="sm:flex gap-6">
          <div className="left-side w-full  md:w-[calc(100%-320px)]">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {article.data?.slice(0, article.data.length)?.map((item) => (
                <ArticleCard
                  key={item.ArticleId}
                  data={item}
                  articledatalength={article.data.length}
                />
              ))}
            </div>
            <div className="flex justify-center w-full   ">
              <div className="mt-10 flex justify-center sm:flex sm:justify-end">
                <NewPagination
                  maxPage={maxPage}
                  page={page}
                  pageChange={pageChange}
                />
              </div>
            </div>
            <div className="py-4">
              <div className="  m-auto sm:mb-0 mb-2 sm:mt-0 mt-2 flex justify-center items-center">
                <GoogleAd_728x90
                  isMobile={props.props.isMobile}
                  path="/22637491760/timesascent.com_erelego_cp_d_728x90"
                  ads_Id="div-gpt-ad-1674644273822-0"
                />
              </div>
            </div>
          </div>
          <div className=" right-side   md:w-[300px]">
            <div className=" ml-2 xl:w-[300px] mb-2">
              <FreelancerWidget {...freelancerWidgetData} />
            </div>

            <div className="sm:mr-[8px] w-[300px] sm:mt-6 md:ml-[8px] h-[250px] m-auto bg-gray-300 flex items-center text-center">
              <GoogleAd_300x250
                path="/22637491760/timesascent.com_erelego_d_ap_300x250"
                ads_Id="div-gpt-ad-1674643828785-0"
                size={[[300, 250]]}
              />
            </div>
            <div>
              <div className="mt-4   sm:mt-6 right-side md:ml-[8px] md:w-[300px]">
                <SecondryWidget
                  iconUrl={`${process.env.Live_API_URL}/recruiterpostjob/staticGrowth.svg/2022/08/18/1660816388.svg`}
                  text2="Events"
                  text3="Looking out to Host your Event Online & Get the Desired Audience"
                  buttonText="List Your Event"
                  href="/listyourevent"
                />
              </div>
              <div className="sm:mr-[8px] md:ml-[8px] w-[300px] h-[250px] mt-4 sm:mt-6 m-auto bg-gray-300 flex items-center text-center">
                <GoogleAd_300x250
                  path="/22637491760/timesascent.com_erelego_d_ap_300x250_1"
                  ads_Id="div-gpt-ad-1674643874881-0"
                  size={[[300, 250]]}
                />
              </div>
            </div>
          </div>
        </div>
      </div>
      <Footer />
    </div>
  );
};
export async function getServerSideProps(context) {
  const pageType = () => {
    if ("featured-articles" === context.query.id) {
      return "Featured";
    } else if ("recommended-read" === context.query.id) {
      return "Most Viewed";
    } else if ("voice-of-hr" === context.query.id) {
      return "HR";
    } else if ("career-development" === context.query.id) {
      return "Career";
    } else if ("interview-tips" === context.query.id) {
      return "interview";
    } else if ("job-interview-tips" === context.query.id) {
      return "interview";
    }
  };
  let userAgent;
  if (context) {
    userAgent = context.req.headers["user-agent"];
  } else {
    userAgent = navigator.userAgent;
  }
  let isMobile = Boolean(
    userAgent.match(
      /Android|BlackBerry|iPhone|iPad|iPod|Opera Mini|IEMobile|WPDesktop/i
    )
  );
  const page = context.query.page ? context.query.page : 1;
  const articledata = await SearchArticlesResponse({
    page: page,
    SearchTerm: pageType(),
  });

  const props = {
    articledata,
    page,
    isMobile,
  };

  return { props: { props } };
}

export default FeaturedArticles;
