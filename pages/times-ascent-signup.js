...
const Signup = () => {
  ...
  const [errorMsgForSignUp, setErrorMsgForSignUp] = useState("");
  ...
  const handleSignup = async (evt) => {
    ...
    await fetch(`${process.env.Live_API_URL}/v1/api/apiTimes/SignUp`, {
      method: "POST",
      body: JSON.stringify({
        countryCode: "91",
        Email: signupemail,
      ...
      .then((response) => {
        if (response.status_code === 200 || response.status_code === 201) {
          window.gtag("event", "conversion", {
            send_to: "AW-10817401996/nBNqCJDQ3YYDEIzpkaYo",
          });
  ...
  const checkUserDetail = async (e) => {
    ...
    await fetch(
      `${process.env.Live_API_URL}/v1/api/apiTimes/checkUserMobile`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        if (
          result.message ===
          "Mobile Number and Email already registered. Please Login"
        ) {
          setEmailIssue(true);
          setMobileIssue(true);
        }
        if (result.message === "Email already registered. Please Login") {
          setEmailIssue(true);
        }
        if (
          result.message === "Mobile Number already registered. Please Login"
        ) {
          setMobileIssue(true);
        }
        if (result.message === "New User" && result.status_code === 200) {
          SendOtpForSignup(e);
        }
      })
      .catch((error) => console.log("error", error))
  ...
  const SendOtpForSignup = async (evt) => {
    evt.preventDefault();

    if (!onVerifyNewPassword(password, signupConfPassword)) {
      setErrorMsgForSignUp("Both password's should be same");
      return;
    }

    sendOtp(signupmobile);
    setLoading(true);
  };
  ...
  const verifyOtp = async (mobile, type) => {
    setLoading(true);
    var myHeaders = new Headers();
    myHeaders.append(
      "Authorization",
      ...
      .then((result) => {
        if (type === "signup") {
          ...
          if (result.status_code === 200) {
            ...
            setErrorMsgForForgotPassword2(result.message);
          } else {
            ApiErrorLog(raw, result, "NewVerifyOtp")
            setErrorMsgForForgotPassword2("Otp Verification Failed");
          }
        }
      })
      .catch((error) => {
                  ...
                  <div className="bg-white py-8 w-full px-4 shadow sm:rounded-lg sm:px-10">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="mt-1">
                            <input
                              ...
                              required
                              onChange={(e) => {
                                setSignupConfPassword(e.target.value);
                        ...
                        {errorMsgForSignUp && (
                          <p className="text-sm text-red-500 rounded-md p-2 font-semibold bg-red-100 mt-2">
                            {errorMsgForSignUp}
                          </p>
                        )}
                        {emailIssue && (
                        ...
                        {mobileIssue && (
                          <div className="text-sm text-red-500 font-semibold rounded-md p-2 bg-red-100 mt-2">
                            Mobile already exist. Try signing in{" "}
                          </div>
                        )}
                        {usererror ? (
                          <p className="p-2 bg-red-200 text-sm text-red-500 font-semibold rounded-md">
                            {usererror}
                          </p>
                        ) : (
                          ""
                  ...
                  <div className="bg-white py-8 w-full px-4 shadow sm:rounded-lg sm:px-10">
                    ...
                    <div>
                      <form
                        ...
                        onSubmit={(e) => {
                          e.preventDefault();
                          verifyOtp(signupmobile, "signup");
                        }}
                      >
                        <div>
                          <label className="block text-sm text-center  font-bold text-gray-700">
                        ...
                        <p className="mx-auto text-center text-sm  ">
                          ...
                          <span
                            ...
                            onClick={() => {
                              setSuccessMsg(sendOtp(signupmobile));
                            }}
                          >
                            Resend OTP
                          </span>
